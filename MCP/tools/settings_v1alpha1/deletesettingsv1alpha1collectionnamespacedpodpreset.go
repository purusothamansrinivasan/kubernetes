...
func Deletesettingsv1alpha1collectionnamespacedpodpresetHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/settings.k8s.io/v1alpha1/namespaces/%s/podpresets%s", cfg.BaseURL, queryString)
...
func CreateDeletesettingsv1alpha1collectionnamespacedpodpresetTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_settings.k8s.io_v1alpha1_namespaces_namespace_podpresets",
		mcp.WithDescription("delete collection of PodPreset"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletesettingsv1alpha1collectionnamespacedpodpresetHandler(cfg),
	}
}
