...
func Listschedulingv1alpha1priorityclassHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateListschedulingv1alpha1priorityclassTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apis_scheduling.k8s.io_v1alpha1_priorityclasses",
		mcp.WithDescription("list or watch objects of kind PriorityClass"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Listschedulingv1alpha1priorityclassHandler(cfg),
	}
}
