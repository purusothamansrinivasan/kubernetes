...
func Deleteappsv1collectionnamespaceddaemonsetHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/apps/v1/namespaces/%s/daemonsets%s", cfg.BaseURL, queryString)
...
func CreateDeleteappsv1collectionnamespaceddaemonsetTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_apps_v1_namespaces_namespace_daemonsets",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deleteappsv1collectionnamespaceddaemonsetHandler(cfg),
	}
}
