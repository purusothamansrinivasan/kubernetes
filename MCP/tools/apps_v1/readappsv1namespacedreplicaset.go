...
func Readappsv1namespacedreplicasetHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/apps/v1/namespaces/%s/replicasets/%s%s", cfg.BaseURL, queryString)
...
func CreateReadappsv1namespacedreplicasetTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apis_apps_v1_namespaces_namespace_replicasets_name",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Readappsv1namespacedreplicasetHandler(cfg),
	}
}
