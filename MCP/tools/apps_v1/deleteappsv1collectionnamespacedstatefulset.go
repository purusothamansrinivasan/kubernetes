...
func Deleteappsv1collectionnamespacedstatefulsetHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/apps/v1/namespaces/%s/statefulsets%s", cfg.BaseURL, queryString)
...
func CreateDeleteappsv1collectionnamespacedstatefulsetTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_apps_v1_namespaces_namespace_statefulsets",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deleteappsv1collectionnamespacedstatefulsetHandler(cfg),
	}
}
