...
func Deletebatchv2alpha1collectionnamespacedcronjobHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/batch/v2alpha1/namespaces/%s/cronjobs%s", cfg.BaseURL, queryString)
...
func CreateDeletebatchv2alpha1collectionnamespacedcronjobTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_batch_v2alpha1_namespaces_namespace_cronjobs",
		mcp.WithDescription("delete collection of CronJob"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletebatchv2alpha1collectionnamespacedcronjobHandler(cfg),
	}
}
