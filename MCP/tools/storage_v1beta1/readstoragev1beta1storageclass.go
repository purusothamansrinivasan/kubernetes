...
func Readstoragev1beta1storageclassHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/storage.k8s.io/v1beta1/storageclasses/%s%s", cfg.BaseURL, queryString)
...
func CreateReadstoragev1beta1storageclassTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apis_storage.k8s.io_v1beta1_storageclasses_name",
		mcp.WithDescription("read the specified StorageClass"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Readstoragev1beta1storageclassHandler(cfg),
	}
}
