...
func Deletecorev1collectionnamespacedconfigmapHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/configmaps%s", cfg.BaseURL, queryString)
...
func CreateDeletecorev1collectionnamespacedconfigmapTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_v1_namespaces_namespace_configmaps",
		mcp.WithDescription("delete collection of ConfigMap"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletecorev1collectionnamespacedconfigmapHandler(cfg),
	}
}
