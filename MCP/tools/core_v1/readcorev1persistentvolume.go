...
func Readcorev1persistentvolumeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/persistentvolumes/%s%s", cfg.BaseURL, queryString)
...
func CreateReadcorev1persistentvolumeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_persistentvolumes_name",
		mcp.WithDescription("read the specified PersistentVolume"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Readcorev1persistentvolumeHandler(cfg),
	}
}
