...
func Readcorev1namespacedsecretHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/secrets/%s%s", cfg.BaseURL, queryString)
...
func CreateReadcorev1namespacedsecretTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_namespaces_namespace_secrets_name",
		mcp.WithDescription("read the specified Secret"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Readcorev1namespacedsecretHandler(cfg),
	}
}
