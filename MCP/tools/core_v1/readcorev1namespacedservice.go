...
func Readcorev1namespacedserviceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/services/%s%s", cfg.BaseURL, queryString)
...
func CreateReadcorev1namespacedserviceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_namespaces_namespace_services_name",
		mcp.WithDescription("read the specified Service"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Readcorev1namespacedserviceHandler(cfg),
	}
}
