...
func Deletecorev1collectionnamespacedresourcequotaHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/resourcequotas%s", cfg.BaseURL, queryString)
...
func CreateDeletecorev1collectionnamespacedresourcequotaTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_v1_namespaces_namespace_resourcequotas",
		mcp.WithDescription("delete collection of ResourceQuota"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletecorev1collectionnamespacedresourcequotaHandler(cfg),
	}
}
