...
func Listcorev1namespacedpersistentvolumeclaimHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateListcorev1namespacedpersistentvolumeclaimTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_namespaces_namespace_persistentvolumeclaims",
		mcp.WithDescription("list or watch objects of kind PersistentVolumeClaim"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Listcorev1namespacedpersistentvolumeclaimHandler(cfg),
	}
}
