...
func Deletecorev1collectionnamespacedreplicationcontrollerHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/replicationcontrollers%s", cfg.BaseURL, queryString)
...
func CreateDeletecorev1collectionnamespacedreplicationcontrollerTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_v1_namespaces_namespace_replicationcontrollers",
		mcp.WithDescription("delete collection of ReplicationController"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletecorev1collectionnamespacedreplicationcontrollerHandler(cfg),
	}
}
