...
func Readcorev1namespacedlimitrangeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/limitranges/%s%s", cfg.BaseURL, queryString)
...
func CreateReadcorev1namespacedlimitrangeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_namespaces_namespace_limitranges_name",
		mcp.WithDescription("read the specified LimitRange"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Readcorev1namespacedlimitrangeHandler(cfg),
	}
}
