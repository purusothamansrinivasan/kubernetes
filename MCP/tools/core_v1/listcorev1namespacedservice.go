...
func Listcorev1namespacedserviceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/services%s", cfg.BaseURL, queryString)
...
func CreateListcorev1namespacedserviceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_namespaces_namespace_services",
		mcp.WithDescription("list or watch objects of kind Service"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Listcorev1namespacedserviceHandler(cfg),
	}
}
