...
func Deletecorev1collectionnamespacedserviceaccountHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/serviceaccounts%s", cfg.BaseURL, queryString)
...
func CreateDeletecorev1collectionnamespacedserviceaccountTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_v1_namespaces_namespace_serviceaccounts",
		mcp.WithDescription("delete collection of ServiceAccount"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletecorev1collectionnamespacedserviceaccountHandler(cfg),
	}
}
