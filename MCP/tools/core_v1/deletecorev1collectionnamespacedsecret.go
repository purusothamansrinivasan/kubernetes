...
func Deletecorev1collectionnamespacedsecretHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/secrets%s", cfg.BaseURL, queryString)
...
func CreateDeletecorev1collectionnamespacedsecretTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_v1_namespaces_namespace_secrets",
		mcp.WithDescription("delete collection of Secret"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletecorev1collectionnamespacedsecretHandler(cfg),
	}
}
