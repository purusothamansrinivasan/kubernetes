...
func Listcorev1persistentvolumeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateListcorev1persistentvolumeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_persistentvolumes",
		mcp.WithDescription("list or watch objects of kind PersistentVolume"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Listcorev1persistentvolumeHandler(cfg),
	}
}
