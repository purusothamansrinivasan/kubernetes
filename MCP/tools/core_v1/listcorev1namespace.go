...
func Listcorev1namespaceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces%s", cfg.BaseURL, queryString)
...
func CreateListcorev1namespaceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_namespaces",
		mcp.WithDescription("list or watch objects of kind Namespace"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Listcorev1namespaceHandler(cfg),
	}
}
