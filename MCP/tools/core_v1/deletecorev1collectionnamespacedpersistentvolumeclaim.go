...
func Deletecorev1collectionnamespacedpersistentvolumeclaimHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/persistentvolumeclaims%s", cfg.BaseURL, queryString)
...
func CreateDeletecorev1collectionnamespacedpersistentvolumeclaimTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_v1_namespaces_namespace_persistentvolumeclaims",
		mcp.WithDescription("delete collection of PersistentVolumeClaim"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletecorev1collectionnamespacedpersistentvolumeclaimHandler(cfg),
	}
}
