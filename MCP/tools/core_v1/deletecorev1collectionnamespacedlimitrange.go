...
func Deletecorev1collectionnamespacedlimitrangeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/limitranges%s", cfg.BaseURL, queryString)
...
func CreateDeletecorev1collectionnamespacedlimitrangeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_v1_namespaces_namespace_limitranges",
		mcp.WithDescription("delete collection of LimitRange"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletecorev1collectionnamespacedlimitrangeHandler(cfg),
	}
}
