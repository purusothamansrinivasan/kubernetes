...
func Deletecorev1namespacedreplicationcontrollerHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletecorev1namespacedreplicationcontrollerTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_v1_namespaces_namespace_replicationcontrollers_name",
		mcp.WithDescription("delete a ReplicationController"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletecorev1namespacedreplicationcontrollerHandler(cfg),
	}
}
