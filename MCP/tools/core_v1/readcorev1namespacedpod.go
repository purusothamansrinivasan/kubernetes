...
func Readcorev1namespacedpodHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/pods/%s%s", cfg.BaseURL, queryString)
...
func CreateReadcorev1namespacedpodTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_namespaces_namespace_pods_name",
		mcp.WithDescription("read the specified Pod"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Readcorev1namespacedpodHandler(cfg),
	}
}
