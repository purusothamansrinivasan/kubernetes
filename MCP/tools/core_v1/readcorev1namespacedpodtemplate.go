...
func Readcorev1namespacedpodtemplateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/podtemplates/%s%s", cfg.BaseURL, queryString)
...
func CreateReadcorev1namespacedpodtemplateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_namespaces_namespace_podtemplates_name",
		mcp.WithDescription("read the specified PodTemplate"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Readcorev1namespacedpodtemplateHandler(cfg),
	}
}
