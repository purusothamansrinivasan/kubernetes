...
func Deletecorev1persistentvolumeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletecorev1persistentvolumeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_v1_persistentvolumes_name",
		mcp.WithDescription("delete a PersistentVolume"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletecorev1persistentvolumeHandler(cfg),
	}
}
