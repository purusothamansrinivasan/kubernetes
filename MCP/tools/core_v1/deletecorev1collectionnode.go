...
func Deletecorev1collectionnodeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/nodes%s", cfg.BaseURL, queryString)
...
func CreateDeletecorev1collectionnodeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_v1_nodes",
		mcp.WithDescription("delete collection of Node"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletecorev1collectionnodeHandler(cfg),
	}
}
