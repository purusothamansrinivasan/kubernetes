...
func Listcorev1namespacedsecretHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateListcorev1namespacedsecretTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_namespaces_namespace_secrets",
		mcp.WithDescription("list or watch objects of kind Secret"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Listcorev1namespacedsecretHandler(cfg),
	}
}
