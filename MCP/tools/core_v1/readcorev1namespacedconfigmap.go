...
func Readcorev1namespacedconfigmapHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/configmaps/%s%s", cfg.BaseURL, queryString)
...
func CreateReadcorev1namespacedconfigmapTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_namespaces_namespace_configmaps_name",
		mcp.WithDescription("read the specified ConfigMap"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Readcorev1namespacedconfigmapHandler(cfg),
	}
}
