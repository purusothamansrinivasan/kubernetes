...
func Readcorev1namespaceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s%s", cfg.BaseURL, queryString)
...
func CreateReadcorev1namespaceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_namespaces_name",
		mcp.WithDescription("read the specified Namespace"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Readcorev1namespaceHandler(cfg),
	}
}
