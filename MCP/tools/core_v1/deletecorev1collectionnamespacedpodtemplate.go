...
func Deletecorev1collectionnamespacedpodtemplateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/podtemplates%s", cfg.BaseURL, queryString)
...
func CreateDeletecorev1collectionnamespacedpodtemplateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_v1_namespaces_namespace_podtemplates",
		mcp.WithDescription("delete collection of PodTemplate"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletecorev1collectionnamespacedpodtemplateHandler(cfg),
	}
}
