...
func Readcorev1namespacedendpointsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/endpoints/%s%s", cfg.BaseURL, queryString)
...
func CreateReadcorev1namespacedendpointsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_namespaces_namespace_endpoints_name",
		mcp.WithDescription("read the specified Endpoints"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Readcorev1namespacedendpointsHandler(cfg),
	}
}
