...
func Readcorev1nodeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/nodes/%s%s", cfg.BaseURL, queryString)
...
func CreateReadcorev1nodeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_v1_nodes_name",
		mcp.WithDescription("read the specified Node"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Readcorev1nodeHandler(cfg),
	}
}
