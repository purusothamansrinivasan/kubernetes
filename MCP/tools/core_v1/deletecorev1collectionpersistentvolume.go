...
func Deletecorev1collectionpersistentvolumeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/persistentvolumes%s", cfg.BaseURL, queryString)
...
func CreateDeletecorev1collectionpersistentvolumeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_v1_persistentvolumes",
		mcp.WithDescription("delete collection of PersistentVolume"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletecorev1collectionpersistentvolumeHandler(cfg),
	}
}
