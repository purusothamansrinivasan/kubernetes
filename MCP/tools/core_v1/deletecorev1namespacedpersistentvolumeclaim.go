...
func Deletecorev1namespacedpersistentvolumeclaimHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateDeletecorev1namespacedpersistentvolumeclaimTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_v1_namespaces_namespace_persistentvolumeclaims_name",
		mcp.WithDescription("delete a PersistentVolumeClaim"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletecorev1namespacedpersistentvolumeclaimHandler(cfg),
	}
}
