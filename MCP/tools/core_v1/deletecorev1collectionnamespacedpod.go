...
func Deletecorev1collectionnamespacedpodHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/pods%s", cfg.BaseURL, queryString)
...
func CreateDeletecorev1collectionnamespacedpodTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_api_v1_namespaces_namespace_pods",
		mcp.WithDescription("delete collection of Pod"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletecorev1collectionnamespacedpodHandler(cfg),
	}
}
