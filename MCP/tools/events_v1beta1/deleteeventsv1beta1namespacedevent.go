...
func Deleteeventsv1beta1namespacedeventHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/events.k8s.io/v1beta1/namespaces/%s/events/%s%s", cfg.BaseURL, queryString)
...
func CreateDeleteeventsv1beta1namespacedeventTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_events.k8s.io_v1beta1_namespaces_namespace_events_name",
		mcp.WithDescription("delete an Event"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deleteeventsv1beta1namespacedeventHandler(cfg),
	}
}
