...
func Deletenetworkingv1collectionnamespacednetworkpolicyHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/networking.k8s.io/v1/namespaces/%s/networkpolicies%s", cfg.BaseURL, queryString)
...
func CreateDeletenetworkingv1collectionnamespacednetworkpolicyTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_networking.k8s.io_v1_namespaces_namespace_networkpolicies",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletenetworkingv1collectionnamespacednetworkpolicyHandler(cfg),
	}
}
