...
func Deletebatchv1collectionnamespacedjobHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/batch/v1/namespaces/%s/jobs%s", cfg.BaseURL, queryString)
...
func CreateDeletebatchv1collectionnamespacedjobTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_batch_v1_namespaces_namespace_jobs",
		mcp.WithDescription("delete collection of Job"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deletebatchv1collectionnamespacedjobHandler(cfg),
	}
}
