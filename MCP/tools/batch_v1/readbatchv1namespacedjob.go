...
func Readbatchv1namespacedjobHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/batch/v1/namespaces/%s/jobs/%s%s", cfg.BaseURL, queryString)
...
func CreateReadbatchv1namespacedjobTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apis_batch_v1_namespaces_namespace_jobs_name",
		mcp.WithDescription("read the specified Job"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Readbatchv1namespacedjobHandler(cfg),
	}
}
