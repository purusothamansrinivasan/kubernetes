...
func Deleterbacauthorizationv1beta1namespacedroleHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/rbac.authorization.k8s.io/v1beta1/namespaces/%s/roles/%s%s", cfg.BaseURL, queryString)
...
func CreateDeleterbacauthorizationv1beta1namespacedroleTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_rbac.authorization.k8s.io_v1beta1_namespaces_namespace_roles_name",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deleterbacauthorizationv1beta1namespacedroleHandler(cfg),
	}
}
