...
func Listrbacauthorizationv1beta1namespacedroleHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/rbac.authorization.k8s.io/v1beta1/namespaces/%s/roles%s", cfg.BaseURL, queryString)
...
func CreateListrbacauthorizationv1beta1namespacedroleTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apis_rbac.authorization.k8s.io_v1beta1_namespaces_namespace_roles",
		mcp.WithDescription("list or watch objects of kind Role"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Listrbacauthorizationv1beta1namespacedroleHandler(cfg),
	}
}
