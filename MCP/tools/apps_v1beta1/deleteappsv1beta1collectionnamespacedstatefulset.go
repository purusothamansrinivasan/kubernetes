...
func Deleteappsv1beta1collectionnamespacedstatefulsetHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/apps/v1beta1/namespaces/%s/statefulsets%s", cfg.BaseURL, queryString)
...
func CreateDeleteappsv1beta1collectionnamespacedstatefulsetTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_apps_v1beta1_namespaces_namespace_statefulsets",
		mcp.WithDescription("delete collection of StatefulSet"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deleteappsv1beta1collectionnamespacedstatefulsetHandler(cfg),
	}
}
