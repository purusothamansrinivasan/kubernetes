...
func Deleterbacauthorizationv1alpha1clusterrolebindingHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/rbac.authorization.k8s.io/v1alpha1/clusterrolebindings/%s%s", cfg.BaseURL, queryString)
...
func CreateDeleterbacauthorizationv1alpha1clusterrolebindingTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_rbac.authorization.k8s.io_v1alpha1_clusterrolebindings_name",
		mcp.WithDescription("delete a ClusterRoleBinding"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deleterbacauthorizationv1alpha1clusterrolebindingHandler(cfg),
	}
}
