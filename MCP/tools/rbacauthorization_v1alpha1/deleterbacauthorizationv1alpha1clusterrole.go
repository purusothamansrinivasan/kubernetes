...
func Deleterbacauthorizationv1alpha1clusterroleHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/rbac.authorization.k8s.io/v1alpha1/clusterroles/%s%s", cfg.BaseURL, queryString)
...
func CreateDeleterbacauthorizationv1alpha1clusterroleTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_rbac.authorization.k8s.io_v1alpha1_clusterroles_name",
		mcp.WithDescription("delete a ClusterRole"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deleterbacauthorizationv1alpha1clusterroleHandler(cfg),
	}
}
