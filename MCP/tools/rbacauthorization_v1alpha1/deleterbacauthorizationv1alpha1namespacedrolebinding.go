...
func Deleterbacauthorizationv1alpha1namespacedrolebindingHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/rbac.authorization.k8s.io/v1alpha1/namespaces/%s/rolebindings/%s%s", cfg.BaseURL, queryString)
...
func CreateDeleterbacauthorizationv1alpha1namespacedrolebindingTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_rbac.authorization.k8s.io_v1alpha1_namespaces_namespace_rolebindings_name",
		mcp.WithDescription("delete a RoleBinding"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deleterbacauthorizationv1alpha1namespacedrolebindingHandler(cfg),
	}
}
