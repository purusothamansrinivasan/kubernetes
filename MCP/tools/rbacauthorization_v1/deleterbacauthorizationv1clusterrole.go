...
func Deleterbacauthorizationv1clusterroleHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/rbac.authorization.k8s.io/v1/clusterroles/%s%s", cfg.BaseURL, queryString)
...
func CreateDeleterbacauthorizationv1clusterroleTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_rbac.authorization.k8s.io_v1_clusterroles_name",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deleterbacauthorizationv1clusterroleHandler(cfg),
	}
}
