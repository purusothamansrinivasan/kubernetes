...
func Listappsv1beta2namespacedstatefulsetHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateListappsv1beta2namespacedstatefulsetTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apis_apps_v1beta2_namespaces_namespace_statefulsets",
		mcp.WithDescription("list or watch objects of kind StatefulSet"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Listappsv1beta2namespacedstatefulsetHandler(cfg),
	}
}
