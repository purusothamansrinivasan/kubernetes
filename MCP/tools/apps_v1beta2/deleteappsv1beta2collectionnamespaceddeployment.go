...
func Deleteappsv1beta2collectionnamespaceddeploymentHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/apps/v1beta2/namespaces/%s/deployments%s", cfg.BaseURL, queryString)
...
func CreateDeleteappsv1beta2collectionnamespaceddeploymentTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_apps_v1beta2_namespaces_namespace_deployments",
	...
	return models.Tool{
		Definition: tool,
		Handler:    Deleteappsv1beta2collectionnamespaceddeploymentHandler(cfg),
	}
}
